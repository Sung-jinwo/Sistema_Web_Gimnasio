...
@section('page-subtitle', 'Renovaciones y contacto por vencimiento de membresías')

@section('content')
<div class="container mx-auto px-4 py-6">
    @can('cobranza.ver')
        <div class="flex justify-end mb-4">
            <a href="{{ route('cobranza.index') }}" class="inline-flex items-center px-4 py-2 bg-gray-800 text-white rounded-lg hover:bg-gray-900 transition">
                <i class="fas fa-hand-holding-dollar mr-2"></i> Ir a Cobranza
            </a>
        </div>
    @endcan
        ...
        <div class="p-4">
            <form method="GET" action="{{ route('seguimiento.index') }}" class="flex flex-col lg:flex-row gap-3">
                ...
                <select name="mes" class="px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-pink-500 focus:border-transparent">
                    @for($i = 1; $i <= 12; $i++)
                        <option value="{{ $i }}" {{ ($filtros['mes'] ?? '') == $i ? 'selected' : '' }}>{{ \Carbon\Carbon::create(null, $i, 1)->locale('es')->translatedFormat('F') }}</option>
                    @endfor
                </select>

                <select name="anio" class="px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-pink-500 focus:border-transparent">
            ...
            <table data-card="compacta" class="min-w-full divide-y divide-gray-200">
                <thead class="bg-gray-50">
                    <tr>
                        ...
                        <th data-card-prioritario class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Vencimiento</th>
                        <th data-card-oculto class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider hidden lg:table-cell">Sede</th>
